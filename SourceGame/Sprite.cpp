...
void Sprite::InitFromFile(const char* folderPath)
{
	/* tìm đường dẫn tilesheet và matrix object */
	string folderPathString = (string)folderPath;

	string imagePath = folderPathString;
	imagePath.append("/image.png");

	string infoPath = folderPathString;
	infoPath.append("/info.dat");

	InitFromFile(imagePath.c_str(),
		infoPath.c_str());
}
...
void Sprite::InitFromImage(const char* imagePath)
{
	...
	frame->left = 0;
	frame->top = 0;
	frame->right = image->Width;
	frame->bottom = image->Height;

	animation->frames._Add(frame);

	this->animations._Add(animation);
}
